  ...
  return (
    <div className="min-h-screen bg-background">
          ...
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-8">
              ...
              <div className="hidden md:flex items-center gap-6">
                ...
                <Link to="/find-mentors" className="text-slate-600 hover:text-slate-900">Find Mentors</Link>
                <Link to="/my-mentors" className="text-slate-600 hover:text-slate-900">My Mentors</Link>
                <Link to="/progress" className="text-slate-600 hover:text-slate-900">Progress</Link>
                <Link to="/mentee-resources" className="text-blue-600 font-medium">Resources</Link>
              </div>
            </div>
            <div className="flex items-center gap-4">
              ...
              <div className="w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center text-white text-sm font-semibold">JD</div>
            </div>
          </div>
        </div>
        <ChatPortal />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-8">
          <div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            ...
            <motion.div key={index} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.08 }} className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <div className="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center"><stat.icon className="w-6 h-6 text-blue-600" /></div>
          ...
          {[
            ...
            <button
              ...
              className={`px-5 py-3 rounded-xl font-medium whitespace-nowrap transition-all ${
                selectedTab === tab.id ? "bg-blue-600 text-white shadow-sm" : "bg-white border border-slate-200 text-slate-600 hover:text-slate-900"
              }`}
            >
              {tab.label}
            </button>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm mb-8">
          <div className="flex flex-col lg:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <input type="text" placeholder="Search your resources..." className="w-full pl-12 pr-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" />
            </div>
        ...
        {selectedTab !== "exercises" && (
          <div className="grid gap-6 mb-10">
            {filteredResources.map((resource, index) => (
              <motion.div key={resource.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.08 }} className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                    ...
                    <div>
                      <div className="flex flex-wrap items-center gap-3 mb-2">
                        ...
                        <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                          resource.status === "Completed" ? "bg-green-100 text-green-700" : 
                          resource.status === "In Progress" ? "bg-blue-100 text-blue-700" : "bg-amber-100 text-amber-700"
                        }`}>{resource.status}</span>
                      </div>
                      <div className="flex flex-wrap items-center gap-4 text-sm text-slate-500 mb-4">
                        <div className="flex items-center gap-1"><Target className="w-4 h-4" />{resource.category}</div>
                        <div className="flex items-center gap-1"><Users className="w-4 h-4" />{resource.mentor}</div>
                        <div className="flex items-center gap-1"><Calendar className="w-4 h-4" />{resource.dueDate}</div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    ...
                    <button className="inline-flex items-center gap-2 px-5 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 shadow-sm">
                      {resource.type === "Podcast" ? <Mic className="w-4 h-4" /> : resource.type.includes("Video") ? <PlayCircle className="w-4 h-4" /> : <BookOpen className="w-4 h-4" />}
                      {resource.type === "Podcast" ? "Listen Now" : resource.type.includes("Book") ? "Read Book" : "Open Resource"}
                    </button>
                  </div>
                </div>
              </motion.div>
        ...
        {selectedTab === "exercises" && (
          <div className="grid lg:grid-cols-2 gap-6">
            {exercises.map((exercise, index) => (
              <motion.div key={index} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.08 }} className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-5">
                  ...
                  <div className={`px-3 py-1 rounded-full text-xs font-semibold ${exercise.completed ? "bg-green-100 text-green-700" : "bg-blue-100 text-blue-700"}`}>
                ...
                <button className="w-full inline-flex items-center justify-center gap-2 px-5 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors font-medium shadow-sm">
                  {exercise.completed ? <><CheckCircle className="w-5 h-5" /> Review Answers</> : <><ChevronRight className="w-5 h-5" /> Start Exercise</>}
                </button>
              </motion.div>
            ))}
          </div>
